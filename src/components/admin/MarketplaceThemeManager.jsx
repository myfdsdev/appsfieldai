...
export default function MarketplaceThemeManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (recordId) {
        await base44.entities.StorePageDefault.update(recordId, { themeThumbnails: thumbnails });
      } else {
        const created = await base44.entities.StorePageDefault.create({ key: "default", themeThumbnails: thumbnails });
        setRecordId(created.id);
      }
      toast.success("Theme previews saved.");
    } catch {
      toast.error("Could not save theme previews. Try again.");
    }
    setSaving(false);
  };
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      <Card className="border-border/40 bg-[#1a1a1a]">
        <CardHeader className="pb-3">
          ...
          <p className="text-xs text-muted-foreground mt-1">Upload a full-page screenshot for each store theme. Store owners see a <span className="text-foreground font-medium">Preview</span> button on the theme in the picker that opens this image full-width and scrollable.</p>
        </CardHeader>
        <CardContent className="space-y-3">
          {STORE_STYLES.map(s => (
            <div key={s.slug} className="p-3 rounded-xl bg-secondary/20">
              <p className="text-xs font-medium text-foreground mb-2 flex items-center gap-1.5">
                ...
                {s.name} <span className="text-muted-foreground font-normal">— {s.tagline}</span>
              </p>
              <R2ImageUpload
          ...
          <div className="pt-3 border-t border-border/30 flex justify-end">
            <Button onClick={handleSave} disabled={saving} className="bg-gradient-to-r from-violet-600 to-cyan-600 rounded-xl gap-1.5"><Save className="w-4 h-4" />Save Theme Previews</Button>
          </div>
        </CardContent>
      </Card>
    </motion.div>
